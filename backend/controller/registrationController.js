...
// ─── Create Registration (Public Accessible) ───────────────────────────────────
const createRegistration = async (req, res) => {
  try {
    ...
    const registration = await Registration.create({
      ...
      examDate,
      // Safely handles public registrations where req.user doesn't exist
      createdBy: req.user?.name || req.user?.role || "Self-Registered", 
    });
...
const updateRegistration = async (req, res) => {
  try {
    ...
    // Added optional chaining (?.) to prevent crashes if req.user is ever missing here
    Object.assign(registration, { ...req.body, updatedBy: req.user?.name || req.user?.role || "System Admin" });
